...
def encrypt_str_to_list(s: str, password=None):
    ...
    lines.extend(split_str_to_list(safe_data))
    return lines


def decrypt_list_to_str(list_in, password=None) -> str:
...
def encrypt_txtfile(fin, fout=None, password=None, skip_password=False):
    """
    example::
    ...
    if password:
        encrypted_list = encrypt_str_to_list(s_raw, password)
        encrypted_list[0] = HIDDEN_PASSWORD_TAG
    else:
        encrypted_list = encrypt_str_to_list(s_raw)
